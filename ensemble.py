...
folders = ["model_1", "model_2", "model_3"]

for folder in folders:
    path = os.path.join(folder, "")
    jsonlist = glob.glob(path + "*.json")
    for json_path in jsonlist:
        file_name = json_path[8:-5]+"_new"
        json_dict = calculate_normalized_voc_given_json_path(json_path)
...
def get_original_images_id_list(file_name):
    json_path = os.path.join("model_1", file)
    f = open(json_path)
    json_dict = json.load(f)
    original_images_ids = []
    for org_img in json_dict["images"]:
...
# file_list=["EndoCV_DATA1_new.json","EndoCV_DATA2_new.json","EndoCV_DATA3_new.json"]
file = "EndoCV_DATA1_new.json"
folder_list = ["model_1", "model_2", "model_3"]
...
with open(file, "w") as outfile:
    json.dump(fusion_dict, outfile)  ##str verince oldu
